...
class DownTxt:
    ...
    def find_list(self):
        ...
        while True:
            try:
                ...
                self.browser.implicitly_wait(10)
                for item in self.browser.find_elements(By.CSS_SELECTOR, self.domain_config.BOOK_LIST):
                    yield item.get_attribute('href'), item.text
                if self.domain_config.BOOK_LIST_NEXT:
                    next_url = self.browser.find_element(By.CSS_SELECTOR, self.domain_config.BOOK_LIST_NEXT).get_attribute('href')
                else:
                    break
    ...
    async def open_tab(self):
        ...
        urls = self.sql.read_db(f"SELECT url FROM `{self.book_name}` WHERE body IS NULL and url != \"END\"")
                ...
                try:
                    ...
                    if _body:
                        ...
                        if self.domain_config.BOOK_CONTENT_NEXT:  # 有分页
                            ...
                            try:
                                self.browser.implicitly_wait(0.1)
                                next_url = self.browser.find_element(By.CSS_SELECTOR,
                                                                     self.domain_config.BOOK_CONTENT_NEXT).get_attribute('href')
                            except NoSuchElementException:
                                next_url = None

                            if is_url(next_url) and idd in next_url:
                                logger.info(f'分页: {next_url}')
                                self.browser.execute_script(f'window.open("{next_url}","_blank");')
                                self.browser.close()
                                continue
                            else:
                                self.body_queue.put((idd, replace(self.cache_dict.pop(idd))))
                                self.browser.close()
    ...
    def write_body_sql(self):
        ...
        while True:
            ...
            try:
                ...
                _rows = self.sql.write_rows(f"UPDATE `{self.book_name}` SET body=%(b)s WHERE idd=%(u)s", all_queue)
                self.body_queue.task_done()
                logger.info('写入正文成功：影响 %d 行, (Body列队长度: %d, Cache Body长度： %d)',
                            _rows, self.body_queue.qsize(), len(self.cache_dict))
